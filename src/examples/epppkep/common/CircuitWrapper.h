//
// Created by timmbirka on 12.10.21. Thanks to Lennart Braun and Amos Treiber for the code.
//
...
public:
  ...
  //added by timmbirka on 14.10.2021
  share_p PutDummySIMDINGate(uint32_t num, size_t bitlen) {
    return share_p(circ_->PutDummySIMDINGate(num, bitlen));
  }
  ...
  // Added by timmbirka on 13.10.2021
  // Only for BooleanCircuits
...
// Added by timm birka on 08.11.2021
// the larger element is stored at index 1
std::vector<share_p> PutCondSwapGate(share_p a, share_p b, share_p s) {
  auto result = ((BooleanCircuit*) circ_)->PutCondSwapGate(a.get(), b.get(), s.get(), false);

  std::vector<share_p> res;
  res.push_back(share_p(result[0]));
  res.push_back(share_p(result[1]));
  return res;
}
// private:
  Circuit* circ_;
